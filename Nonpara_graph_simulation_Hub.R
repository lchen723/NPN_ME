...
Theta = XMRF.Sim(n=100,p, model="GGM",graph.type="hub")$B*0.5
...
# Gaussian simulation
# -----------------------
simulate_gaussian <- function(p, n) {
  t(XMRF.Sim(n,p, model="GGM",graph.type="hub")$X)
}
...
Theta[1:5,1:5]
pm=PM/re
pm[which(abs(pm)<0.05)]=0
pm0=PM0/re
pm0[which(abs(pm0)<0.05)]=0
pm_glasso = PM_glasso/re
pm_glasso[which(abs(pm_glasso)<0.05)]=0
pm_GUEST = PM_GUEST/re
pm_GUEST[which(abs(pm_GUEST)<0.05)]=0
pm_Kendall = PM_Kendall/re
pm_Kendall[which(abs(pm_Kendall)<0.05)]=0
pm_Spearman = PM_Spearman/re
pm_Spearman[which(abs(pm_Spearman)<0.05)]=0
#PM[which(PM<0.3)]=0
pm_clime = PM_clime/re
pm_clime[which(abs(pm_clime)<0.05)]=0
...
sum(abs(pm_glasso - Theta)^2)
sum(abs(pm_clime - Theta)^2)
sum(abs(pm_GUEST - Theta)^2)
sum(abs(pm_Spearman - Theta)^2)
sum(abs(pm_Kendall - Theta)^2)
sum(abs(pm0 - Theta)^2)
sum(abs(pm - Theta)^2)


GR = function(Thetahat,Theta0) {
SPE = 0; SEN = 0
p0 = dim(Theta0)[1]
for(i in 1:p0) {
for(j in 1:p0) {
SPE = SPE + (Thetahat[i,j]!=0 & Theta0[i,j]!=0)*1
SEN = SEN + (Thetahat[i,j]==0 & Theta0[i,j]==0)*1
}
}
return(c(SPE/length(which(Theta0!=0)), SEN/(p0^2-length(which(Theta0!=0)))   )    )
